package net.zeeraa.novacore.module;
...
import net.zeeraa.novacore.module.event.ModuleDisabledEvent;
import net.zeeraa.novacore.module.event.ModuleEnableEvent;
	...
	public boolean enable() {
			...
			for (Class<? extends NovaModule> clazz : dependencies) {
				...
				if (!ModuleManager.isEnabled(clazz)) {
					if (!ModuleManager.enable(clazz)) {
						Log.error("Failed to load module " + this.getName() + ". Failed to enable dependency" + clazz.getName());
		...
		try {
			...
			if (this instanceof Listener) {
				Log.info("Registering listeners for module " + this.getName());
