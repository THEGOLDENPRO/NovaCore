...
public class VersionIndependentUtils extends net.zeeraa.novacore.spigot.abstraction.VersionIndependentUtils {
	...
	public DeathType getDeathTypeFromDamage(EntityDamageEvent e, Entity lastDamager) {
		Location blockx = e.getEntity().getLocation();
		blockx.setX(Math.ceil(blockx.getX()));
		blockx.setY(Math.floor(blockx.getY()));
		blockx.setZ(Math.floor(blockx.getZ()));

		Location blockxnegative = e.getEntity().getLocation();
		blockxnegative.setX(Math.floor(blockx.getX()));
		blockxnegative.setY(Math.floor(blockx.getY()));
		blockxnegative.setZ(Math.floor(blockx.getZ()));

		Location blockz = e.getEntity().getLocation();
		blockz.setX(Math.ceil(blockx.getX()));
		blockz.setY(Math.floor(blockx.getY()));
		blockz.setZ(Math.floor(blockx.getZ()));

		Location blockznegative = e.getEntity().getLocation();
		blockznegative.setX(Math.floor(blockx.getX()));
		blockznegative.setY(Math.floor(blockx.getY()));
		blockznegative.setZ(Math.floor(blockx.getZ()));

		e.getEntity().getWorld().getBlockAt()
		switch (e.getCause()) {
			case FIRE:
				if (lastDamager != null) {
